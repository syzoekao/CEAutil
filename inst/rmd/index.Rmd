...
* If you have external datasets, you could load/read or save/write the datasets. 
* Some common functions include `load()` and `readRDS()` for generic `R` data types (e.g., `.RData`, `.rda`, and `.rds`). 
...
`r text_spec("Need discussion! Should we introduce base plot or ggplot2?", background = "#D05A6E", color = "white", bold = T)`
