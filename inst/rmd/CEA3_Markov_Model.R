...
ages <- c(10 : (10 + l_param_all$n_t - 1))

# We extract the probability of natural death age 10-110 from the lifetable.
p_mort <- ltable$qx[match(ages, other_input_ls$ltable$age)]
...
p_init_tan <- other_input_ls$behavior$p_init_tanning[match(ages, other_input_ls$behavior$age)]
p_halt_tan <- other_input_ls$behavior$p_halt_tanning[match(ages, other_input_ls$behavior$age)]

# We get the number of health states based on the length of the string vector, state_names
state_names <- l_param_all$state_names
n_states <- length(l_param_all$state_names)
...
tr_mat["nontan", "cancer", ] <- l_param_all$p_nontan_to_cancer
tr_mat["nontan", "deadnature", ] <- p_mort
tr_mat["nontan", "nontan", ] <- 1 - p_init_tan - l_param_all$p_nontan_to_cancer - p_mort

# 2. Fill out the transition probabilities from regular tanner to other states
tr_mat["regtan", "nontan", ] <- p_halt_tan
tr_mat["regtan", "cancer", ] <- l_param_all$p_regtan_to_cancer
tr_mat["regtan", "deadnature", ] <- p_mort
tr_mat["regtan", "regtan", ] <- 1 - p_halt_tan - l_param_all$p_regtan_to_cancer - p_mort
...
tr_mat["cancer", "deadcancer", ] <- l_param_all$p_cancer_to_dead
tr_mat["cancer", "deadnature", ] <- p_mort
tr_mat["cancer", "nontan", ] <- 1 - l_param_all$p_cancer_to_dead - p_mort
...
trace_mat[1, ] <- l_param_all$v_init
